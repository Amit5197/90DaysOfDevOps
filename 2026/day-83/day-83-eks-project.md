...
```
![image](images/install_envoy_gw.png)
...
```
![image](images/external_address.png)
...
helm install monitoring prometheus-community/kube-prometheus-stack \
  ...
  --wait --timeout 600s
```
![image](images/helm_monitoring_stack.png)

Verify:
```bash
kubectl get pods -n monitoring
```
![image](images/verify_monitor_pods.png)
...
![image](images/jvm_memory_usage.png)

# HTTP request rate
rate(http_server_requests_seconds_count{namespace="bankapp"}[5m])

![image](images/http_req_rate.png)
...
![image](images/http_req_latency.png)
...
![image](images/ns_bankapp.png)


![image](images/bankapp_pod.png)


![image](images/mysql_pod.png)


![image](images/ollama_pod.png)
...
kubectl get pvc -A
```
![image](images/k8s_obj_teardown.png)
...
![image](images/infra_teardown.png)
...
![image](images/flow_dig.png)

**Key takeaways from the 3-day EKS block**

* Set up a production Kubernetes cluster on Amazon EKS using Terraform, including networking, nodes, and access configuration

* Deployed and managed a full application (app + database + AI service) using kubectl, with autoscaling and persistent storage via Amazon EBS

* Handled traffic routing and security using Gateway API and Envoy, including HTTPS and session handling

* Monitored and validated the system using metrics and dashboards, ensuring everything works reliably in a real cloud setup
